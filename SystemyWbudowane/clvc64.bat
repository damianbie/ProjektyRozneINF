@echo off
rem @echo Setting environment for using Microsoft Visual Studio 2010 x64 tools.
...
@set LIB=.\Lib;%LIB%
echo Kompilator Visual Studio 2010 (x64). Budowanie...
cl.exe /nologo /LD %1 /link /DEF:outcode.def /OUT:%2
@rem cl.exe /nologo /LD %1 /link /DEF:outcode.def /OUT:.\Temp\outcode.dll
@rem cl.exe /nologo /LD %1 /link /DEF:..\outcode.def /OUT:outcode.dll
@if %ERRORLEVEL% LEQ 1 (
   goto good
)
...
:good
   @echo.
   echo Budowanie DLL zakoñczone sukcesem.
   goto build_end
